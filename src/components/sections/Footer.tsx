import Image from "next/image";
import { Mail, MapPin } from "lucide-react";

import { footer } from "@/lib/content";
import { images } from "@/lib/assets";

const TITLE_FONT = "var(--font-urbanist), Urbanist, sans-serif";
const BODY_FONT = "var(--font-public-sans), Public Sans, sans-serif";

/* Brand-accurate social glyphs (lucide-react in this project lacks these).
   Single-path simple-icons shapes on a 24x24 viewBox, filled currentColor. */
const SOCIAL_GLYPHS: Record<string, string> = {
  instagram:
    "M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163C8.741 0 8.332.014 7.052.072 2.695.272.272 2.69.072 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z",
  x: "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231 5.45-6.231zm-1.161 17.52h1.833L7.084 4.126H5.117l11.966 15.644z",
  linkedin:
    "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.063 2.063 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z",
  facebook:
    "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z",
};

function SocialGlyph({ name, size = 27 }: { name: string; size?: number }) {
  const d = SOCIAL_GLYPHS[name.toLowerCase()];
  if (!d) return null;
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d={d} />
    </svg>
  );
}

export function Footer() {
  return (
    <footer className="bg-[#251913] text-white">
      <div className="mx-auto w-full max-w-[1240px] px-6 sm:px-8 lg:px-[100px]">
        <div className="py-16 lg:py-[100px]">
          {/* Header: logo + social cluster */}
          <div className="flex flex-col gap-10 sm:flex-row sm:items-start sm:justify-between">
            <Image
              src={images.logo}
              alt="PodHub"
              width={112}
              height={32}
              className="h-[56px] w-auto lg:h-[68px]"
              priority={false}
            />
            <div className="flex flex-col gap-6">
              <span
                style={{
                  fontFamily: TITLE_FONT,
                  fontWeight: 600,
                  fontSize: 20,
                  lineHeight: "30px",
                  letterSpacing: "-0.4px",
                  color: "#FFFFFF",
                }}
              >
                We&rsquo;re on social
              </span>
              <div className="flex items-center gap-5 text-white">
                {footer.socials.map((s) => (
                  <a
                    key={s.name}
                    href={s.href}
                    aria-label={s.name}
                    className="transition hover:text-[#F17F3F]"
                  >
                    <SocialGlyph name={s.name} size={27} />
                  </a>
                ))}
              </div>
            </div>
          </div>

          {/* Divider */}
          <div className="my-16 h-px w-full bg-[#4E5255]" aria-hidden />

          {/* Footer info: contact block + link columns */}
          <div className="flex flex-col gap-12 lg:flex-row lg:justify-between">
            {/* Contact */}
            <div className="lg:max-w-[366px]">
              <div className="flex flex-col gap-[19px]">
                <a
                  href={`mailto:${footer.contact.email}`}
                  className="flex items-center gap-4 transition hover:text-[#F17F3F]"
                >
                  <Mail size={24} strokeWidth={1.6} />
                  <span
                    style={{
                      fontFamily: BODY_FONT,
                      fontWeight: 400,
                      fontSize: 16,
                      lineHeight: "24px",
                      letterSpacing: "-0.64px",
                    }}
                  >
                    {footer.contact.email}
                  </span>
                </a>
                <div className="flex items-start gap-4">
                  <MapPin size={24} strokeWidth={1.6} className="mt-[2px] shrink-0" />
                  <span
                    className="whitespace-pre-line"
                    style={{
                      fontFamily: BODY_FONT,
                      fontWeight: 400,
                      fontSize: 16,
                      lineHeight: "24px",
                      letterSpacing: "-0.64px",
                    }}
                  >
                    {footer.contact.addressLines.join("\n")}
                  </span>
                </div>
              </div>
              <p
                className="mt-[61px]"
                style={{
                  fontFamily: BODY_FONT,
                  fontWeight: 400,
                  fontSize: 16,
                  lineHeight: "24px",
                  letterSpacing: "-0.64px",
                  color: "#CFD0D1",
                }}
              >
                {footer.copyright}
              </p>
            </div>

            {/* Link columns */}
            <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-4 sm:gap-x-8 lg:gap-x-[66px]">
              {footer.columns.map((col) => (
                <div key={col.title} className="flex flex-col">
                  <h4
                    style={{
                      fontFamily: TITLE_FONT,
                      fontWeight: 600,
                      fontSize: 20,
                      lineHeight: "30px",
                      letterSpacing: "-0.4px",
                      color: "#F17F3F",
                    }}
                  >
                    {col.title}
                  </h4>
                  <ul className="mt-4 flex flex-col gap-3">
                    {col.links.map((link) => (
                      <li key={link.label}>
                        <a
                          href={link.href}
                          className="transition hover:text-white"
                          style={{
                            fontFamily: BODY_FONT,
                            fontWeight: 400,
                            fontSize: 16,
                            lineHeight: "24px",
                            letterSpacing: "-0.64px",
                            color: "#E7E7E8",
                          }}
                        >
                          {link.label}
                        </a>
                      </li>
                    ))}
                  </ul>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
